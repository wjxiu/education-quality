...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    public List<TeacherRateSituationResp> getStudentRateSituation(List<Integer> teacherIds) {
        List<StudentEvalDO> list = studentEvalService.list(new LambdaQueryWrapper<StudentEvalDO>().in(teacherIds!=null&& !teacherIds.isEmpty(),StudentEvalDO::getTeacherId, teacherIds).likeRight(StudentEvalDO::getCreateTime, LocalDate.now().getYear()));
        Map<Integer, String> teacherIdToName = teacherService.list(new LambdaQueryWrapper<TeacherDO>().in(teacherIds != null&&!teacherIds.isEmpty(), TeacherDO::getId, teacherIds)).stream().collect(Collectors.toMap(TeacherDO::getId, TeacherDO::getRealName));
        Map<Integer, List<StudentEvalDO>> teacherIdToList = list.stream().collect(Collectors.groupingBy(StudentEvalDO::getTeacherId));
        ...
        for (Map.Entry<Integer, List<StudentEvalDO>> teacherInfo : teacherIdToList.entrySet()) {
            ...
            Map<Integer, List<StudentEvalDO>> evalIdToList = studentEvalDOS.stream().collect(Collectors.groupingBy(StudentEvalDO::getEvalId));
            ...
            for (Map.Entry<Integer, List<StudentEvalDO>> entry : evalIdToList.entrySet()) {
                List<Double> rateItemList = entry.getValue().stream().map(item -> item.getRate().doubleValue()).toList();
            ...
            resp1.setList(items);resp1.setTeacherName( teacherIdToName.get(teacherInfo.getKey()));
            resp1.setTeacherId(teacherInfo.getKey());resp1.setMode(mode);resp1.setMedian(median);resp1.setMean(mean1);
            resp.add(resp1);
        }
        return resp;
    }
    ...
    public List<String> getStudentRemainCoursRate(List<Integer> studentIds) {
        ...
        List<StudentEvalDO> studentEvalDOS = studentEvalService.list(new LambdaQueryWrapper<StudentEvalDO>().in(studentIds!=null&&!studentIds.isEmpty(),StudentEvalDO::getStuId, studentIds).likeRight(StudentEvalDO::getCreateTime, LocalDate.now().getYear()));
        Map<Integer, List<StudentEvalDO>> collect1 = studentEvalDOS.stream().collect(Collectors.groupingBy(StudentEvalDO::getStuClassId));
