...
public class DepartmentServiceImpl extends ServiceImpl<DepartmentMapper, DepartmentDO>
    ...
    @Override
    public List<String> getallName(String majorName) {
        if (majorName == null || majorName.equals("")) {
            return list().stream().map(DepartmentDO::getName).toList();
        }
        return majorService.list(new LambdaQueryWrapper<MajorDO>().select(MajorDO::getDepartmentName).eq(MajorDO::getMajorName, majorName)).stream().map(MajorDO::getDepartmentName).toList();
    }
