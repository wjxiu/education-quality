...
public class TeacherServiceImpl extends ServiceImpl<TeacherMapper, TeacherDO>
    ...
    public LoginResp login(Integer id, String password) {
        ...
        String token = JWTUtil.generateAccessToken(new UserInfoDTO(id, teacherDO.getRealName(), null));
        LoginResp loginResp = new LoginResp();
        loginResp.setId(id);
        loginResp.setName(teacherDO.getRealName());loginResp.setToken(token);
        return loginResp;
    }
    @Override
    public Integer register(TeacherDO teacherDO) {
        if (StringUtils.hasLength(teacherDO.getPhone())&&teacherMapper.selectCount(new LambdaQueryWrapper<TeacherDO>().eq(TeacherDO::getPhone,teacherDO.getPhone()))>0) throw new ClientException("手机号重复");
        if (StringUtils.hasLength(teacherDO.getEmail())&&teacherMapper.selectCount(new LambdaQueryWrapper<TeacherDO>().eq(TeacherDO::getEmail,teacherDO.getEmail()))>0) throw new ClientException("邮箱重复");
